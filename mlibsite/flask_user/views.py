...
from flask import current_app, flash, redirect, request, url_for, Markup
...
def _do_login_user(user, safe_next, remember_me=False):
    ...
    if user_manager.enable_email and user_manager.enable_confirm_email \
        ...
        # Ilya Flash with link
        flash(Markup(f'Ваш электронный адрес не был подтвержден. Поищите письмо в папках Входящие и Спам или <a href="{url}">повторно получите письмо</a>.'), 'error')
